...
class PdoChoc {
    ...
    public function Michlo1() {
      
        $requetePrepare = PdoChoc::$monPdo->prepare(
                'SELECT * FROM produit WHERE id IN ("herisson","plateau_traiteur", "berlingots","cadeau_convive_sachet","outils_chocolat_noir")'
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll(PDO::FETCH_ASSOC);
    }    
    
        public function Michlo2() {
      
        $requetePrepare = PdoChoc::$monPdo->prepare(
                'SELECT * FROM produit WHERE id IN ("citrouille","mendiants","bonbons_espelette", "cadeau_convive_enfant", "rose_chocolat")'
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll(PDO::FETCH_ASSOC);
    }  
        ...
        public function Michlo3() {
      
        $requetePrepare = PdoChoc::$monPdo->prepare(
                'SELECT * FROM produit WHERE id IN ("choc_o_lait","ballotins","bonbons_seve_pin", "dragees_petits_coeurs", "escarpin_chocolat_rose")'
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll(PDO::FETCH_ASSOC);
    }  
        ...
        public function Michlo4() {
      
        $requetePrepare = PdoChoc::$monPdo->prepare(
                'SELECT * FROM produit WHERE id IN ("pralines_roses","boite_de_luxe", "gateau_bonbons","grelons","escarpin_chocolat_rose")'
        );
        $requetePrepare->execute();
        return $requetePrepare->fetchAll(PDO::FETCH_ASSOC);
    }  
